...
const HomeScreen_store: React.FC<HomeScreenProps> = ({
  storeName,
  storeId,
}) => {
  // 상태 관리
  ...
  // 현재 월 데이터 처리 상태
  const [currentMonthWeeklyData, setCurrentMonthWeeklyData] = useState<{
    [week: string]: CombinedOrderData[];
  }>({});
  const [currentMonthWeeklyTotals, setCurrentMonthWeeklyTotals] = useState<{
    [week: string]: number;
  }>({});
  const [currentMonthSortedWeeks, setCurrentMonthSortedWeeks] = useState<
    string[]
  >([]);
  const [currentMonthProductSummary, setCurrentMonthProductSummary] = useState<
    Array<{
      품목명: string;
      총수량: number;
      총금액: number;
  ...
  // 지난 달 데이터 처리 상태
  const [lastMonthWeeklyData, setLastMonthWeeklyData] = useState<{
    [week: string]: CombinedOrderData[];
  }>({});
  const [lastMonthWeeklyTotals, setLastMonthWeeklyTotals] = useState<{
    [week: string]: number;
  }>({});
    ...
    try {
      ...
      if (itemsResponse.ok) {
        ...
        if (currentMonthResponse.ok) {
          ...
          const currentMonthOrdersData: StoreOrderData[] =
            currentMonthResult.orders;
  ...
  const processMonthlyData = (
    ...
    setWeeklyData: React.Dispatch<
      React.SetStateAction<{ [week: string]: CombinedOrderData[] }>
    >,
    setWeeklyTotals: React.Dispatch<
      React.SetStateAction<{ [week: string]: number }>
    >,
    setSortedWeeks: React.Dispatch<React.SetStateAction<string[]>>,
    setProductSummary: React.Dispatch<
        ...
        <>
          ...
          <ScrollView>
            ...
            <View
              ...
              style={homescreenStyles.sectionContainer}
            >
              ...
              lastMonthOrders.length === 0 ? (
                <Text testID="noDataText" style={homescreenStyles.noDataText}>
                  발주 내역이 없습니다.
                </Text>
              ) : (
                <>
                  ...
                  Object.values(lastMonthWeeklyTotals).every(
                    ...
                    <Text
                      testID="noDataText"
                      style={homescreenStyles.noDataText}
                    >
                      발주 내역이 없습니다.
                    </Text>
                  ) : (
                    <View
                      testID="chartDataContainer"
                      style={homescreenStyles.chartDataContainer}
                    >
                      <View
                        testID="chartContainer"
                        style={homescreenStyles.chartContainer}
                      >
                        <Text style={homescreenStyles.chartUnitText}>
                          (단위: 만원)
                        </Text>
                        <BarChart
                      ...
                      <View
                        ...
                        style={[homescreenStyles.summarySection]}
                      >
                        <View
                          testID="summaryTable"
                          style={homescreenStyles.summaryTable}
                        >
                          <View
                            testID="tableRow"
                            style={homescreenStyles.tableRow}
                          >
                            <View
                              testID="amountCell"
                              style={[
                            ...
                            .map((week, index) => (
                              <View
                                ...
                                style={[
                                  ...
                                  index % 2 === 0
                                    ? homescreenStyles.tableRowEven
                                    : homescreenStyles.tableRowOdd,
                                ]}
                              >
                                <Text
                                  testID="summaryLabel"
                                  style={[
                                    homescreenStyles.summaryLabel,
                                    {
                                      flex: 1,
                                      textAlign: "left",
                          ...
                          >
                            <Text
                              ...
                              style={[
                                homescreenStyles.summaryTotalLabel,
                                { flex: 1, paddingLeft: moderateScale(10) },
                              ]}
                            >
                              월발주금액
                            </Text>
                            ...
                            >
                              <Text
                                ...
                                style={[
                                  homescreenStyles.summaryTotalValue,
                                  {
                                    fontSize: RFValue(14),
                                    color: "#0D326F",
                                    fontWeight: "600",
                          ...
                          <View
                            testID="comparisonContainer"
                            style={homescreenStyles.comparisonContainer}
                          >
                            <View
                              testID="comparisonItem"
                              style={homescreenStyles.comparisonItem}
                            >
                              <Text
                                testID="comparisonLabel"
                                style={homescreenStyles.comparisonLabel}
                              >
                                전월 총 발주금액
                              </Text>
                              <Text
                                testID="comparisonValue"
                                style={homescreenStyles.comparisonValue}
                              >
                                {f.formatPrice(lastMonthTotal)}원
                              </Text>
                            </View>
              ...
              {currentMonthOrders.length === 0 ? (
                ...
                >
                  <View
                    ...
                    style={homescreenStyles.monthlyTableHeader}
                  >
                    <View
                      testID="productColumn"
                      style={homescreenStyles.productColumn}
                    >
                      <Text
                        testID="monthlyTableHeaderText"
                        style={[
                          homescreenStyles.monthlyTableHeaderText,
                          { textAlign: "left" },
                        ]}
                        numberOfLines={1}
                        ellipsizeMode="tail"
                    ...
                    {currentMonthSortedWeeks.map((week) => (
                      <View
                        key={week}
                        testID="weekColumn"
                        style={homescreenStyles.weekColumn}
                      >
                        <Text
                          testID="monthlyTableHeaderText"
                          style={homescreenStyles.monthlyTableHeaderText}
                          numberOfLines={1}
                          ellipsizeMode="tail"
                        >
                          {week}주
                      ...
                      >
                        ...
                        >
                          <Text
                            ...
                            style={[
                              homescreenStyles.monthlyTableCell,
                              { textAlign: "left" },
                            ]}
                            numberOfLines={2}
                            ellipsizeMode="tail"
                        ...
                        {currentMonthSortedWeeks.map((week) => {
                          ...
                          return (
                            ...
                            >
                              <Text
                                testID="monthlyTableCell"
                                style={homescreenStyles.monthlyTableCell}
                                numberOfLines={2}
                                ellipsizeMode="tail"
                              >
