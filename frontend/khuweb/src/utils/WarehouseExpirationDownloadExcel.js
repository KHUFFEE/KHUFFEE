...
export const warehouseExpirationDownloadExcel = ({ tableRows }) => {
  ...
  // ③ 헤더 행 (칼럼: 협력사, 품목명, 유통기한, 개수, 현재고, 남은 일수)
  const headerRow = [
    "협력사",
    "품목명",
    "유통기한",
  ...
  tableRows.forEach((row) => {
    ...
    excelRow[3] = row.count;
    excelRow[4] = row.currentStock;
    excelRow[5] = row.remainingDays;
  ...
  // ⑦ Header Title 영역에 StoreOrdersDownloadExcel.js처럼 외부 테두리 적용 (얇은 선)
  for (let r = 0; r < 2; r++) {
  ...
  // ⑨ 데이터 행 기본 스타일 (Arial, 크기 10, 얇은 테두리)
  ...
  for (let r = 4; r <= fullRange.e.r; r++) {
    for (let c = 0; c < totalCols; c++) {
      ...
      if (ws[cellAddr]) {
        ws[cellAddr].s = ws[cellAddr].s || {};
        // '개수'(col 3)와 '현재고'(col 4)의 숫자 서식 분리 처리
        if (c === 3) {
