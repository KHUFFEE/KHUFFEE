...
class StoreOrderCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if duplicate_qs.exists():
            ...
            store_pk = (
                validated_data["매장_id"].pk
                if hasattr(validated_data["매장_id"], "pk")
                else validated_data["매장_id"]
            )
            item_pk = (
            ...
            with connection.cursor() as cursor:
                ...
                """
                cursor.execute(
                    update_sql, [add_amount, store_pk, item_pk, period_val, round_val]
                )
                select_sql = f"""
                    SELECT 매장_발주량
                    FROM {table_name}
        ...
        if duplicate_qs.exists():
            if new_value == "" or float(new_value) == 0:
                ...
                with connection.cursor() as cursor:
                    ...
                    """
                    cursor.execute(
                        update_sql, [add_amount, store_pk, item_pk, period_val]
                    )
    ...
    def create(self, validated_data):
        ...
        if store_obj.pk != "ST_102":
            raise serializers.ValidationError(
                "매장_id must be ST_102 for WarehouseIncoming."
            )
