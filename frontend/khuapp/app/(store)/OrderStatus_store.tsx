...
import {
  orderStatusStyles,
  dateRangeStyles,
} from "../../src/styles/OrderStatus_styles_store";
  ...
  const handleSearch = () => {
    ...
    fetch(`${RN_API_URL}/api/orders/store_order_list/?order=desc&page=1`)
      ...
      .then((data) => {
        ...
        // 프리셋에 따라 시작 날짜 계산
        const [endYear, endMonth, endWeek] = currentPeriod
          .split(".")
          .map(Number);
        let startPeriod;
        ...
        if (activePreset === "최근 1개월") {
          ...
          const prevMonth =
            endMonth - 3 <= 0 ? endMonth - 3 + 12 : endMonth - 3;
          const prevYear = endMonth - 3 <= 0 ? endYear - 1 : endYear;
          startPeriod = `${prevYear}.${String(prevMonth).padStart(2, "0")}.${endWeek}`;
        } else if (activePreset === "최근 6개월") {
          const prevMonth =
            endMonth - 6 <= 0 ? endMonth - 6 + 12 : endMonth - 6;
      ...
      <View testID="modalOverlay" style={dateRangeStyles.modalOverlay}>
        <View testID="modalContainer" style={dateRangeStyles.modalContainer}>
          <View testID="modalHeader" style={dateRangeStyles.modalHeader}>
            <Text testID="modalTitle" style={dateRangeStyles.modalTitle}>
              기간조회
            </Text>
            <TouchableOpacity
              testID="closeButton"
              onPress={onClose}
              style={dateRangeStyles.closeButton}
            >
              <Text
                testID="closeButtonText"
                style={dateRangeStyles.closeButtonText}
              >
                ✕
              </Text>
            </TouchableOpacity>
          ...
          <ScrollView
            testID="presetButtons"
            horizontal
            contentContainerStyle={dateRangeStyles.presetButtons}
          >
            {presets.map((preset) => (
              <TouchableOpacity
                key={preset}
              ...
              >
                <Text
                  ...
                  style={[
                    ...
                    activePreset === preset &&
                      dateRangeStyles.activePresetButtonText,
                  ]}
                >
                  {preset}
                </Text>
          ...
          {activePreset && (
            <TouchableOpacity
              ...
              style={dateRangeStyles.searchButton}
              onPress={handleSearch}
            >
              <Text
                testID="confirmButtonText"
                style={dateRangeStyles.searchButtonText}
              >
                확인
              </Text>
            </TouchableOpacity>
...
const OrderStatus_store: React.FC<OrderStatusProps> = ({ storeId }) => {
  ...
  const [detailGroupOrders, setDetailGroupOrders] = useState<
    CombinedOrderData[]
  >([]);
  const [detailGroupDate, setDetailGroupDate] = useState<string>("");

  // 월별 상세보기 관련 상태
  const [monthlyDetailModalVisible, setMonthlyDetailModalVisible] =
    useState<boolean>(false);
  const [monthlyDetailOrders, setMonthlyDetailOrders] = useState<
    CombinedOrderData[]
  >([]);
  ...
  const openDetailModal = (dateKey: string, orders: CombinedOrderData[]) => {
    ...
    filteredOrders.forEach((order) => {
      ...
      if (groupedOrdersMap[key]) {
        groupedOrdersMap[key].매장_발주량 =
          (groupedOrdersMap[key].매장_발주량 || 0) + (order.매장_발주량 || 0);
        groupedOrdersMap[key].totalCost =
          (groupedOrdersMap[key].totalCost || 0) + (order.totalCost || 0);
      } else {
    ...
    <View testID="status_container" style={orderStatusStyles.status_container}>
      {!loading && sortedYears.length > 0 && (
        <>
          ...
          <View
            ...
            style={orderStatusStyles.buttonContainer}
          >
            <View
              testID="rightButtonGroup"
              style={orderStatusStyles.rightButtonGroup}
            >
              <TouchableOpacity
                testID="headerButton"
                style={[
              ...
              >
                <Text
                  ...
                  style={[
                    ...
                    sortOrder === "asc" &&
                      orderStatusStyles.headerButtonTextActive,
                  ]}
                >
      ...
      {loading ? (
        <View
          ...
          style={orderStatusStyles.loadingContainer}
        >
            ...
            return (
              <React.Fragment key={year}>
                ...
                {sortedMonths.map((month) => {
                  ...
                  const monthTotalCost = weeksWithNonZeroOrders.reduce(
                    (monthSum, w) => {
                      const ordersInWeek = weeksObj[w];
                  ...
                  return (
                    <View
                      key={month}
                      testID="monthContainer"
                      style={orderStatusStyles.monthContainer}
                    >
                      <View
                        testID="monthHeader"
                        style={orderStatusStyles.monthHeader}
                      >
                        <Text
                          testID="monthTitle"
                          style={orderStatusStyles.monthTitle}
                        >
                          {parseInt(month)}월
                        </Text>
                        <View
                        ...
                        >
                          <Text
                            testID="monthTotal"
                            style={orderStatusStyles.monthTotal}
                          >
                          ...
                          <TouchableOpacity
                            ...
                            style={[
                              orderStatusStyles.detailButton,
                              {
                          ...
                          >
                            <Text
                              testID="monthDetailButtonText"
                              style={orderStatusStyles.detailButtonText}
                            >
                              월별 상세보기
                            </Text>
                          </TouchableOpacity>
                      ...
                      {sortedWeeks.map((week) => {
                        ...
                        return (
                          <View
                            key={week}
                            testID="weekContainer"
                            style={orderStatusStyles.weekContainer}
                          >
                            <View
                              testID="weekHeader"
                              style={orderStatusStyles.weekHeader}
                            >
                              <Text
                                testID="weekTitle"
                                style={orderStatusStyles.weekTitle}
                              >
                                {week}주차
                              </Text>
                              <Text
                                testID="weekTotal"
                                style={orderStatusStyles.weekTotal}
                              >
                            ...
                            <View
                              testID="orderContent"
                              style={orderStatusStyles.orderContent}
                            >
                              <View
                                testID="orderInfo"
                                style={orderStatusStyles.orderInfo}
                              >
                                <Text
                                  testID="productName"
                                  style={orderStatusStyles.productName}
                                >
                                  {firstOrder.품목명}
                                </Text>
                                {extraCount > 0 && (
                                  <Text
                                    testID="extraCount"
                                    style={orderStatusStyles.extraCount}
                                  >
                                    외 {extraCount}개
                                  </Text>
                                )}
                              ...
                              >
                                <Text
                                  testID="detailButtonText"
                                  style={[
                                    orderStatusStyles.detailButtonText,
      ...
      >
        <View
          testID="modalCenteredView"
          style={orderStatusStyles.modalCenteredView}
        >
          <View testID="modalView" style={orderStatusStyles.modalView}>
            <View testID="modalHeader" style={orderStatusStyles.modalHeader}>
              ...
              >
                <Text
                  testID="closeButtonText"
                  style={orderStatusStyles.closeButtonText}
                >
                  X
                </Text>
              </TouchableOpacity>
            ...
            <ScrollView
              ...
              style={[orderStatusStyles.modalScrollView, { flexGrow: 1 }]}
            >
            ...
            <View testID="modalFooter" style={orderStatusStyles.modalFooter}>
              <Text
                testID="modalTotalCost"
                style={orderStatusStyles.modalTotalCost}
              >
                총 발주금액:
              </Text>
              <Text
      ...
      >
        ...
        >
          <View testID="modalView" style={orderStatusStyles.modalView}>
            ...
            >
              <View
                testID="monthlyTableContainer"
                style={orderStatusStyles.monthlyTableContainer}
              >
                <View
                  testID="monthlyTableHeader"
                  style={orderStatusStyles.monthlyTableHeader}
                >
                  <View
                    testID="productColumn"
                    style={orderStatusStyles.productColumn}
                  >
                    <Text
                      testID="monthlyTableHeaderText"
                      style={[
                        orderStatusStyles.monthlyTableHeaderText,
                        { textAlign: "left" },
                      ]}
                      numberOfLines={1}
                      ellipsizeMode="tail"
                  ...
                  {sortedWeeks.map((week) => (
                    <View
                      testID="weekColumn"
                      key={week}
                      style={orderStatusStyles.weekColumn}
                    >
                      <Text
                        testID="monthlyTableHeaderText"
                        style={orderStatusStyles.monthlyTableHeaderText}
                        numberOfLines={1}
                        ellipsizeMode="tail"
                      >
                        {week}주
                  ...
                  <View
                    testID="quantityColumn"
                    style={orderStatusStyles.quantityColumn}
                  >
                    <Text
                      testID="monthlyTableHeaderText"
                      style={orderStatusStyles.monthlyTableHeaderText}
                ...
                {productSummary.map((product, index) => (
                  ...
                  >
                    ...
                    >
                      <Text
                        testID="monthlyTableCell"
                        style={[
                          orderStatusStyles.monthlyTableCell,
                          { textAlign: "left" },
                        ]}
                        numberOfLines={2}
                        ellipsizeMode="tail"
                    ...
                    {sortedWeeks.map((week) => {
                      const weekData = product.주차별[week] || {
                        수량: 0,
                        금액: 0,
                      };
                      return (
                        ...
                        >
                          <Text
                            testID="monthlyTableCell"
                            style={orderStatusStyles.monthlyTableCell}
                            numberOfLines={2}
                            ellipsizeMode="tail"
                          >
                    ...
                    >
                      <Text
                        testID="monthlyTableCellHighlight"
                        style={orderStatusStyles.monthlyTableCellHighlight}
                        numberOfLines={2}
                        ellipsizeMode="tail"
                      >
                        {product.총수량}
              ...
              <View
                ...
                style={orderStatusStyles.summarySection}
              >
                <Text
                  testID="summaryTitle"
                  style={orderStatusStyles.summaryTitle}
                >
                  월 발주 금액 요약
                </Text>
                {sortedWeeks.map((week) => (
                  <View
                    testID="summaryRow"
                    key={week}
                    style={orderStatusStyles.summaryRow}
                  >
                    <Text
                      testID="summaryLabel"
                      style={orderStatusStyles.summaryLabel}
                    >
                      {week}주차 발주금액
                    </Text>
                    <Text
                      testID="summaryValue"
                      style={orderStatusStyles.summaryValue}
                    >
                ...
                <View
                  testID="summaryTotal"
                  style={orderStatusStyles.summaryTotal}
                >
                  <Text
                    testID="summaryTotalLabel"
                    style={orderStatusStyles.summaryTotalLabel}
                  >
                    월 총 발주금액
                  </Text>
                  <Text
                    testID="summaryTotalValue"
                    style={orderStatusStyles.summaryTotalValue}
                  >
                    {f.formatPrice(monthlyTotal)}원
                  </Text>
                </View>
              </View>
            </ScrollView>
